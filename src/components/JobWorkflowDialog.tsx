...
export default function JobWorkflowDialog({ job, open, onOpenChange }: JobWorkflowDialogProps) {
  ...
  const difficultyLabels: Record<string, { label: string; color: string }> = {
    easy: { label: "Easy", color: "text-[var(--jade-bright)]" },
    medium: { label: "Medium", color: "text-primary" },
    hard: { label: "Hard", color: "text-[var(--brass)]" },
        ...
        <ScrollArea className="max-h-[60vh] pr-4">
          {!hasWorkflow ? (
            <div className="text-center py-12 text-muted-foreground">
              <AvaSeal size={48} className="mx-auto mb-4 opacity-50" />
                ...
                <Card className="bg-background border-border">
                  ...
                  <CardContent className="space-y-3">
                    {workflowSteps.map((step, index) => {
                      const StepIcon = STEP_ICONS[step.type] || AvaSeal;
                      return (
