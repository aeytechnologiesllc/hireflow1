...
export function SalesAnalysisDialog({ open, onOpenChange, data }: SalesAnalysisDialogProps) {
  ...
  const getWouldBuyIcon = () => {
    if (evaluation?.wouldBuy === "yes") return <ThumbsUp className="h-5 w-5 text-primary" />;
    if (evaluation?.wouldBuy === "maybe") return <HelpCircle className="h-5 w-5 text-[var(--warning)]" />;
        ...
        <ScrollArea className="max-h-[65vh] pr-4">
          <div className="space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`font-semibold ${
                    evaluation?.wouldBuy === "yes" ? "text-primary" :
                    evaluation?.wouldBuy === "maybe" ? "text-[var(--warning)]" : "text-destructive"
                  }`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all ${
                        (evaluation?.discovery ?? 0) >= 70 ? "bg-primary" :
                        (evaluation?.discovery ?? 0) >= 50 ? "bg-[var(--warning)]" : "bg-destructive"
                      }`}
                ...
                <div className="p-3 bg-muted/50 rounded-lg">
                  ...
                  <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all ${
                        (evaluation?.objectionHandling ?? 0) >= 70 ? "bg-primary" :
                        (evaluation?.objectionHandling ?? 0) >= 50 ? "bg-[var(--warning)]" : "bg-destructive"
                      }`}
                ...
                <div className="p-3 bg-muted/50 rounded-lg">
                  ...
                  <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all ${
                        (evaluation?.valueProposition ?? 0) >= 70 ? "bg-primary" :
                        (evaluation?.valueProposition ?? 0) >= 50 ? "bg-[var(--warning)]" : "bg-destructive"
                      }`}
                ...
                <div className="p-3 bg-muted/50 rounded-lg">
                  ...
                  <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all ${
                        (evaluation?.closingSkills ?? 0) >= 70 ? "bg-primary" :
                        (evaluation?.closingSkills ?? 0) >= 50 ? "bg-[var(--warning)]" : "bg-destructive"
                      }`}
            ...
            {evaluation?.improvements && evaluation.improvements.length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-[var(--warning)] mb-2">Areas for Improvement</h4>
                <ul className="space-y-1.5">
