/**
 * SERVER MIRROR of src/lib/candidateJourney.ts — do not hand-edit the logic
 * below without also updating that file (and vice versa).
 *
 * trustedResults.ts (this directory) needs the exact same "what step is
 * next / has this candidate actually reached this step" rules the client
 * uses, so recordStepResult's server-side phase advance can never disagree
 * with what CandidateStepGate.tsx / useJourneyPosition.ts already decided
 * client-side. Edge functions run on Deno and cannot import from `src/`
 * (different module graph, path aliases, and bundler), so this is a real,
 * separate file rather than a shared package — which means it can drift.
 * scripts/guards/candidate-journey-shared-copy.mjs fails the build if the
 * two files' exported function bodies (normalized) ever stop matching, so
 * a change to one without the other is caught immediately rather than
 * silently reintroducing a two-copies-of-the-truth bug.
 *
 * Below this banner, the file is byte-for-byte the same module doc comment
 * and code as src/lib/candidateJourney.ts.
 */
