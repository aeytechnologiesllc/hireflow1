...
import { callOpenAIChat, openAIErrorStatus } from "../_shared/openai.ts";

// Model is configurable so a retirement is a config change, not a code change.
// Set OPENAI_DOCUMENT_MODEL to the replacement model when swapping.
const OPENAI_API_KEY = Deno.env.get("OPENAI_API_KEY");
const OPENAI_DOCUMENT_MODEL = Deno.env.get("OPENAI_DOCUMENT_MODEL") || "gpt-5.6-luna";
...
Generate the complete, professionally formatted document now:`;
    ...
    console.log(`Generating document: ${documentType} (model: ${OPENAI_DOCUMENT_MODEL})`);
    ...
    try {
      const result = await callOpenAIChat({
        ...
        // The old gateway call sent no cap at all. A full employment contract
        // plus its disclaimer block runs well past the helper's 2000-token
        // default, so give it real headroom rather than truncating mid-clause.
      ...
      console.error("AI API error:", status ?? "network/parse", error);
      ...
      throw status !== null ? new Error(`AI API error: ${status}`) : error;
    }
